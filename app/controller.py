...
from . import db
from .models import Batch, Category, Course, Enquiry, Enrollment, Qualification, User
import json
...
def get_mycourses(userId):
    ...
    rt["main"]=[]
    enrolled = Enrollment.query.filter_by(user_id=userId).all()
    courses=[]
    for i in enrolled:
        if i.status:
    ...
    for j in courses:
        i = Course.query.filter_by(course_id=j).first()
        # name of the instructor
        rt["main"].append({
            "title":i.course_name,
            "sub_title":User.query.filter_by(user_id = Course.query.filter_by(course_id=i.course_id).first().instructor_id).first().full_name,
